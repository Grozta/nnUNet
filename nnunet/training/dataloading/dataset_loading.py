...
class DataLoader3D_with_selected_wieght(DataLoader3D):
    def __init__(self, data, patch_size, final_patch_size, batch_size, dataset_need_focal_class_weight, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None):
        super(DataLoader3D_with_selected_wieght, self).__init__(data, patch_size, final_patch_size, batch_size, has_prev_stage=has_prev_stage,
                 oversample_foreground_percent=oversample_foreground_percent, memmap_mode=memmap_mode, pad_mode=pad_mode, pad_kwargs_data=pad_kwargs_data,
                 pad_sides=pad_sides)
        ...
        #     content = load_json(self.dataset_debug_file)
        # else:
        #     content = []
        # content.append({'new start':'[{}]'.format(datetime.now())})
        # save_json(content,self.dataset_debug_file)
        
    def write_log_line(self, message):
        with open(self.dataset_debug_log, 'a') as file:
    ...
    def generate_train_batch(self):
        ...
        seg = np.zeros(self.seg_shape, dtype=np.float32)
        case_properties = []
        debug_do_oversample_dict = OrderedDict()
        for j, i in enumerate(selected_keys):
            # oversampling foreground will improve stability of model training, especially if many patches are empty
            # (Lung for example)
        ...
        # debug_info['debug_do_oversample_dict'] = debug_do_oversample_dict
        # content = load_json(self.dataset_debug_file) 
        # content.append({'debug_info':debug_info})
        # save_json(content,self.dataset_debug_file)
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys}
